...
static void mbox_write()
{
  ...
  /* Reading CMD via scanf */
  //getchar();

    
  if( ! (scanf("%*[\n]/SEND %s %[ -z]", reciever, message) == 2) ) //ignores the new line character which may still be in the stream. scanf stops reading when it recieves the first new line character. vs getchar();
  {
    fgets(buf,5,stdin);
